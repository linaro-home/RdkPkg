...
#define LABEL_RESERVED_PARTITION    "reserved"
  ...
  )
{
  ...
  Status = RdkReadFile (
              (CONST CHAR16 *)RDK_KEK_CRT_FILE_NAME, 
              (VOID **)&KekCrtData, 
              &KekCrtSize
              );
  ASSERT_EFI_ERROR (Status);
  ...
  PartitionRead(LABEL_RESERVED_PARTITION, Buffer, RSVD_READ_SIZE);
  ...
  )
{
  ...
  Status = RdkReadFile ((CONST CHAR16 *)RDK_FDT_NAME, (VOID**) &FdtData, &FdtDataSize);
  ASSERT_EFI_ERROR (Status);
